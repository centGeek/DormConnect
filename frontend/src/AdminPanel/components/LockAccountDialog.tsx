...
export default function DeleteUserDialog({ user, onSuccess, onError }: { user: UpdateUserDTO, onSuccess?: (msg: string) => void, onError?: (msg: string) => void }) {
  ...
  return (
    <>
      ...
      {!isLocked && 
        <div>
    <button className='w-40 bg-blue-600 m-2 text-white px-5 py-2 rounded-lg hover:bg-blue-500 transition' onClick={() => setIsOpen(true)}>Odblokuj konto użytkownika</button>
